...
public class BridgeCreater : MonoBehaviour
{
    ...
    public void CreateBridge()
    {
        ...
        if (GameManager.ObjectToFollow.Count-1 >= winCondition)
        {
            ...
            for (int i=1; i<=((winCondition/5)+1); i++)
            {
                for(int j = 1; j <= 6; j++)
                {
                ...
                //MoveFriend(GameManager.ObjectToFollow[i], i);
            }
        }
        //Loose
        else
    ...
    private void Update()
    {
        if (isWon && !isPlaced)
        {
            ...
            if (transform.position == target)
            {
                ...
                {
                    GameManager.ObjectToFollow[friendIndex].GetComponent<EnemyAnimatorController>().SetRunningFalse();
                }
            }
        }
        if (isPlaced)
